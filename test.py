...
# #         # Generate a noise value for terrain smoothness
# #         noise_value = simplex.noise2(x / 50, y / 50) + 0.5 * simplex.noise2(x / 30, y / 30) + 0.25 * simplex.noise2(x / 10, y / 10)

# #         # Assign colors based on noise value
# #         if noise_value < -0.3:
# #             color = (139, 69, 19)  # Dark Brown (Barren land)
# #         elif -0.3 <= noise_value < -0.1:
# #             color = (205, 133, 63)  # Sandy Brown (Desert)
# #         elif -0.1 <= noise_value < 0.1:
# #             color = (154, 205, 50)  # Yellow-Green (Transition to fertile land)
# #         elif 0.1 <= noise_value < 0.3:
# #             color = (34, 139, 34)  # Forest Green (Fertile land)
# #         elif 0.3 <= noise_value < 0.5:
# #             color = (144, 238, 144)  # Light Green (Highly fertile land)
# #         else:
# #             color = (70, 130, 180)  # Steel Blue (Water)

# #         previous_terrain[(x, y)] = color  # Cache the value
# #     return previous_terrain.get((x, y), BLACK)  # Default to black if outside radius
...
#                 terrain_color = generate_terrain(x, y, planet_radius)
